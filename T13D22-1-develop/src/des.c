...
int CIPHER[64];
int ENCRYPTED[64];
int DES_encrypt(char* filename);

void expansion_function(int pos, int text) {
    for (int i = 0; i < 48; i++)
        if (E[i] == pos + 1) EXPtext[i] = text;
}
...
void cipher(int Round, int mode) {
    for (int i = 0; i < 32; i++) expansion_function(i, RIGHT[Round - 1][i]);
...
void Encryption(long int plain[], char* filename) {
    out = fopen(filename, "w");
    fclose(out);
    out = fopen(filename, "ab+");

    for (int i = 0; i < 64; i++) initialPermutation(i, plain[i]);
...
void encrypt(long int n, char* filename) {
    char* fn1 = calloc(sizeof(char), 70);
    strcpy(fn1, filename);
    ...
    for (int i = 0; i < n; i++) Encryption(plain + 64 * i, fn1);
...
int DES_encrypt(char* filename) {
    ...
    long int n = findFileSize(filename) / 8;

    convertCharToBit(n, filename);
