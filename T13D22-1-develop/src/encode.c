...
int findFiles(char* str) {
    ...
    int i = strlen(str);
    if (i >= 2) {
        if ((str[i - 1] == 'c') && (str[i - 2] == '.')) res = 'c';
        if ((str[i - 1] == 'h') && (str[i - 2] == '.')) res = 'h';
    }
    return res;
}
...
int codeCaesars(char* path) {
    int is_error = 0;
    int shift;
    if (scanf("%d", &shift) != 1) is_error = 1;
    DIR* dir = opendir(path);
    if (dir == NULL) is_error = 1;
    if (is_error == 0) {
        struct dirent* cur;
