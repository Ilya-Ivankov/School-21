...
int main() {
    ...
    if (is_error == 0) is_error = input(matrix, height, width);
    if (is_error == 0) {
...
int getChoice() {
    int choice;
    if ((scanf("%d", &choice) != 1) || (choice > 3) || choice < 1) choice = -1;
    return choice;
}
...
void dynamicAllocation2(int ***matrix, int height, int width) {
    *matrix = (int **)malloc(sizeof(int *) * height);
    ...
    for (int i = 0; i < height; i++) tmp[i] = (int *)malloc(sizeof(int) * width);
    *matrix = tmp;
}
...
void dynamicAllocation3(int ***matrix, int height, int width) {
    *matrix = (int **)malloc(sizeof(int *) * height);
    ...
    for (int i = 1; i < height; i++) (*matrix)[i] = (*matrix)[0] + width * i;
}
